...
template<class T>
bool halfing_error_T_uniform(const T * P, const T * D, size_t n, const double tau, std::vector<double>& ebs, std::vector<T>& Temp_ori, std::vector<double>& error_est_Temp, std::vector<double>& error_Temp){
	double eb_P = ebs[0];
	double eb_D = ebs[1];
	double R = 287.1;
	double c_1 = 1.0 / R;
	...
	if(max_value > tau){
        ...
        while(estimate_error > tau){
            ...
            eb_P = eb_P / 1.5;
            eb_D = eb_D / 1.5;
            estimate_error = c_1 * MDR::compute_bound_division<double>(P[i], D[i], eb_P, eb_D);
        }
        ebs[0] = eb_P;
        ebs[1] = eb_D;
		return false;
...
void reconstruct_GE(const std::string data_file_prefix, const std::string rdata_file_prefix,
    ...
    std::vector<T> Temp_ori(num_elements, 0);
    MDR::compute_T(vars_vec[0].get(), vars_vec[1].get(), num_elements, Temp_ori.data());
    target_eb *= getRange(Temp_ori.data(), num_elements);
    ...
    while((!tolerance_met) && (iter < max_iter)){
        ...
        tolerance_met = halfing_error_T_uniform(P_dec, D_dec, num_elements, target_eb, targetEBs, Temp_ori, error_est_Temp, error_Temp);
...
void QoI_decompress_preprocess(const std::string data_name, const std::string data_prefix_path,
    ...
    if (std::strcmp(data_name.c_str(), "GE") == 0) {
        ...
        std::cout << "No Temp for " << data_name << " dataset." << std::endl;
    }
    return;                     
}
