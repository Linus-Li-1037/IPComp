...
template<class T>
bool halfing_error_C_uniform(const T * P, const T * D, size_t n, const double tau, std::vector<double>& ebs, std::vector<T>& C_ori, std::vector<double>& error_est_C, std::vector<double>& error_C){
	double eb_P = ebs[0];
	double eb_D = ebs[1];
	double R = 287.1;
	double gamma = 1.4;
	...
	if(max_value > tau){
		...
		while(estimate_error > tau){
			...
			double e_T = c_1 * MDR::compute_bound_division<double>(P[i], D[i], eb_P, eb_D);
			double Temp = P[i] / (D[i] * R);
			estimate_error = c_2*MDR::compute_bound_square_root_x<double>(Temp, e_T);
		}
        ebs[0] = eb_P;
        ebs[1] = eb_D;
		return false;
...
void reconstruct_GE(const std::string data_file_prefix, const std::string rdata_file_prefix,
    ...
    std::vector<T> C_ori(num_elements, 0);
    MDR::compute_C(vars_vec[0].get(), vars_vec[1].get(), num_elements, C_ori.data());
    target_eb *= getRange(C_ori.data(), num_elements);
    ...
    std::vector<double> error_C(num_elements);
    std::vector<double> error_est_C(num_elements);
    ...
    while((!tolerance_met) && (iter < max_iter)){
        ...
        tolerance_met = halfing_error_C_uniform(P_dec, D_dec, num_elements, target_eb, targetEBs, C_ori, error_est_C, error_C);
        max_act_error = print_max_abs(error_C);
        max_est_error = print_max_abs(error_est_C);  
    }
...
void QoI_decompress_preprocess(const std::string data_name, const std::string data_prefix_path,
    ...
    if (std::strcmp(data_name.c_str(), "GE") == 0) {
        ...
        std::cout << "No C for " << data_name << " dataset." << std::endl;
    }
    return;                     
}

void usage(char* cmd) {
    std::cout << "halfing_C usage: " << cmd <<
