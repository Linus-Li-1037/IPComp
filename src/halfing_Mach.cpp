...
template<class T>
bool halfing_error_Mach_uniform(const T * Vx, const T * Vy, const T * Vz, const T * P, const T * D, size_t n, const std::vector<unsigned char>& mask, const double tau, std::vector<double>& ebs, std::vector<T>& Mach_ori, std::vector<double>& error_est_Mach, std::vector<double>& error_Mach){
	double eb_Vx = ebs[0];
	double eb_Vy = ebs[1];
	double eb_Vz = ebs[2];
	double eb_P = ebs[3];
	...
	for(int i=0; i<n; i++){
		...
		// print_error("V_TOT", V_TOT, Mach_ori[i], e_V_TOT);
	...
	if(max_value > tau){
		...
		while(estimate_error > tau){
			...
			double e_C = c_2*MDR::compute_bound_square_root_x<double>(Temp, e_T);
			double C = c_2 * sqrt(Temp);
			estimate_error = MDR::compute_bound_division<double>(V_TOT, C, e_V_TOT, e_C);
		}
		ebs[0] = eb_Vx;
		ebs[1] = eb_Vy;
		ebs[2] = eb_Vz;
...
void reconstruct_GE(const std::string data_file_prefix, const std::string rdata_file_prefix,
    ...
    std::vector<T> Mach_ori(num_elements, 0);
    MDR::compute_Mach(vars_vec[0].get(), vars_vec[1].get(), vars_vec[2].get(), vars_vec[3].get(), vars_vec[4].get(), num_elements, Mach_ori.data());
    target_eb *= getRange(Mach_ori.data(), num_elements);
    ...
    while((!tolerance_met) && (iter < max_iter)){
        ...
        tolerance_met = halfing_error_Mach_uniform(Vx_dec, Vy_dec, Vz_dec, P_dec, D_dec, num_elements, mask, target_eb, targetEBs, Mach_ori, error_est_Mach, error_Mach);
...
void QoI_decompress_preprocess(const std::string data_name, const std::string data_prefix_path,
    ...
    if (std::strcmp(data_name.c_str(), "GE") == 0) {
        ...
        std::cout << "No Mach for " << data_name << " dataset." << std::endl;
    }
    return;                     
}
